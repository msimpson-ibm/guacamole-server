 ...
 */
static int guac_kubevirt_is_address(const char* hostname) {
 ...
 */
static X509* guac_kubevirt_read_cert(char* pem) {
 ...
 */
static int guac_kubevirt_assume_cert_ok(X509_STORE_CTX* x509_ctx, void* arg) {
    return 1;
}
...
void guac_kubevirt_init_ssl(guac_client* client, SSL_CTX* context) {
    ...
    if (settings->ignore_cert) {
        guac_client_log(client, GUAC_LOG_INFO,
                "SSL certificate validation bypassed via custom callback");
        SSL_CTX_set_verify(context, SSL_VERIFY_PEER, NULL);
        SSL_CTX_set_cert_verify_callback(context,
                guac_kubevirt_assume_cert_ok, NULL);
    }
    ...
    /* Otherwise use the given CA certificate to validate (if any) */
        ...
        X509* ca_cert = guac_kubevirt_read_cert(settings->ca_cert);
        if (ca_cert == NULL) {
    ...
    /* Enable hostname checking if not ignoring certificate validation */
    if (!settings->ignore_cert) {
        ...
        if (guac_kubevirt_is_address(settings->hostname)) {
